...
1. [Infrastructure du réseau](#1-infrastructure-du-réseau)
2. [Configuration SNMP sur les serveurs cibles](#2-configuration-snmp-sur-les-serveurs-cibles)
3. [Installation et configuration du serveur de supervision (srv-monitoring)](#3-installation-et-configuration-du-serveur-de-supervision)
4. [Installation et configuration du serveur Linux (srv-linux)](#4-installation-et-configuration-du-serveur-linux)
5. [Configuration du pare-feu pfSense](#5-configuration-du-pare-feu-pfsense)
6. [Configuration du serveur Windows (AK-DC)](#6-configuration-du-serveur-windows)
7. [Supervision avec Zabbix](#7-supervision-avec-zabbix)
8. [Supervision avancée — Sondes et alertes](#8-supervision-avancée--sondes-et-alertes)
9. [SIEM Wazuh — Centralisation des logs](#9-siem-wazuh--centralisation-des-logs)
10. [Simulation d'une attaque par force brute SSH](#10-simulation-dune-attaque-par-force-brute-ssh)
...
> **Figure 1** — `vms_list.png` : Liste des machines virtuelles déployées dans VirtualBox
...
> **Figure 2** — `snmp_walk_on_pfsense_result.png` : Résultat du snmpwalk sur le firewall pfSense  
> **Figure 3** — `ping_from_srv_monitoring_to_srv_linux.png` : Test de connectivité entre srv-monitoring et srv-linux

### 2.3 SNMP sur pfSense (192.168.1.1)
...
> **Figure 4** — `snmp_enabled_on_pfsense.png` : Interface SNMP activée sur pfSense
...
> **Figure 5** — `mariadb_installed_success.png` : MariaDB installé et opérationnel  
> **Figure 6** — `database_schema.png` : Schéma de la base de données Zabbix  
> **Figure 7** — `db_password_added.png` : Mot de passe de la base configuré
...
> **Figure 8** — `zabbix_server_enable_activated.png` : Service Zabbix Server actif  
> **Figure 9** — `zabbix_web_interface.png` : Interface web Zabbix accessible à `http://192.168.1.100/zabbix`
...
> **Figure 10** — `ssh_acces_from_srv_monitoring_to_srv_linux.png` : Accès SSH depuis srv-monitoring vers srv-linux
...
> **Figure 11** — `zabbix_agent_status_on_linux_server.png` : Agent Zabbix actif sur srv-linux
...
> **Figure 12** — `zabbix_get_result.png` : Résultat de zabbix_get sur srv-linux
...
> **Figure 13** — `wazuh_agent_status_on_linux_server.png` : Agent Wazuh v4.7.5 actif sur srv-linux
...
> **Figure 14** — `pfsense_web_interface_login.png` : Page de connexion à l'interface web pfSense  
> **Figure 15** — `pfsense_firewall_terminal_interface.png` : Interface console pfSense
...
> **Figure 16** — `firewall_rule_for_syslog.png` : Règle de pare-feu autorisant le syslog vers srv-monitoring
...
> **Figure 17** — `zabbix_3_hosts_supervised.png` : Les 3 hôtes supervisés dans le tableau de bord Zabbix (srv-linux, AK-DC, firewall)
...
> **Figure 18** — `item_config_for_snmp_wan_interface.png` : Configuration de l'item SNMP pour l'interface WAN  
> **Figure 19** — `network_trafic_after_flood_ping.png` : Trafic réseau visible dans Zabbix après un flood ping
...
> **Figure 20** — `port_80_litening_in_latest_data.png` : Valeur de l'item port 80 dans "Latest data"  
> **Figure 21** — `sup_avance_dashboard_before_simulation.png` : Dashboard avant la simulation
...
> **Figure 22** — `simulation_nginx_stop.png` : Arrêt du service Nginx  
> **Figure 23** — `simulation_status_port_80_0.png` : Alerte déclenchée dans Zabbix — port 80 passe à 0
...
> **Figure 24** — `trigger_rules_critic_files.png` : Règle de trigger pour la surveillance du fichier critique
...
> **Figure 25** — `alert_result_critic_file_removed.png` : Alerte déclenchée dans Wazuh après suppression du fichier
...
> **Figure 26** — `critic_file_removed.png` : Fichier supprimé  
> **Figure 27** — `critic_file_recreated.png` : Fichier recréé  
> **Figure 28** — `alert_resolved_after_critic_file_back.png` : Alerte résolue après recréation du fichier  
> **Figure 29** — `dashboard_after_critic_file_deleted.png` : Dashboard Wazuh après suppression du fichier
...
### 9.2 Installation Wazuh (All-in-One)
...
> **Figure 30** — `wazuh_web_interface.png` : Interface web Wazuh Dashboard  
> **Figure 31** — `curl_acces_on_wazuh_dashboard.png` : Vérification de l'accès au dashboard Wazuh  
> **Figure 32** — `filebeat_output_test.png` : Test de la sortie Filebeat vers l'indexer

### Agents Wazuh enregistrés

Après l'installation et la configuration des agents sur srv-linux et AK-DC :

> **Figure 33** — `wazuh_dashboard_agent_diplayed.png` : Agents Wazuh visibles dans le dashboard  
> **Figure 34** — `2_agents_on_dashboard.png` : Deux agents actifs (srv-linux + AK-DC) dans le tableau de bord  
> **Figure 35** — `agents_securtiy_events.png` : Événements de sécurité remontés par les agents
...
> **Figure 36** — `simutation_attaque_brute_force.png` : Lancement de l'attaque Hydra depuis srv-monitoring  
> **Figure 37** — `top_5_attacks_detected_ssh_brute_force.png` : Wazuh — Top 5 des attaques détectées, dont la force brute SSH
